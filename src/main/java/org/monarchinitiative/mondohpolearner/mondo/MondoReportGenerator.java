...
public class MondoReportGenerator {
	private static final Logger logger = Logger.getLogger(MondoReportGenerator.class.getName());
	...
	public void generateIndexFile() {
		try {
			String indexFilename = MondoProcessor.reportHomeDir + File.separator + "index.md";
			...
			for (String classCurie : classKeyList) {
				...
				File reportFile = new File(MondoProcessor.markdownDir + File.separator  + classCurie.replace(":", "_") + ".md");
	...
	public void render(String classCurie) {
		...
		try{
			...
			FileWriter fw = new FileWriter(MondoProcessor.markdownDir + File.separator + reportFilenameBody + ".md", false);
			BufferedWriter bw = new BufferedWriter(fw);
			PrintWriter out = new PrintWriter(bw);
