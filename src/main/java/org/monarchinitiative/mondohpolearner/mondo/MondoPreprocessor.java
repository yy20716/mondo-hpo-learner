...
public class MondoPreprocessor {
	...
	public MondoPreprocessor(Multimap<String, OWLIndividual> classParamMap, Multimap<String, String> classEqEntityMap,
	...
	private void generateAbox() {
		try {
			...
			File file = new File(MondoProcessor.hpofilewithAbox);
	...
	private void computeClassMappings() {
		logger.info("Computing all subclasses and equivalent entities from " + MondoProcessor.inputOWLFile + "...");
		ResultSet resultSet = QueryExecutor.executeOnce (MondoProcessor.inputOWLFile, MondoProcessor.queryExtractSubclasses);
